...
[MM, GenForce] = equationsToMatrix(eqns,ddq);
...
%%%% Energy:
KineticEnergy = ...
    0.5*m1*dot(dG1,dG1) + 0.5*I1*dq1 + ...
    0.5*m2*dot(dG2,dG2) + 0.5*I2*dq2 + ...
    0.5*m3*dot(dG3,dG3) + 0.5*I3*dq3 + ...
    0.5*m4*dot(dG4,dG4) + 0.5*I4*dq4 + ...
    0.5*m5*dot(dG5,dG5) + 0.5*I5*dq5;
PotentialEnergy = ...
    m1*g*G1(2) + ...
    m2*g*G2(2) + ...
    m3*g*G3(2) + ...
...
Idx = find(MM);  
MassMatrixVec = MM(Idx);  % Column vector of non-zero elements
